...
Por otro lado, vamos a introducir el paquete **lubridate** para trabajar con formatos de fecha.

Por último, vamos a trabajar sobre distintas formas de exportar nuestros resultados e importar bases de datos.
...
base <- eph::get_microdata(year = 2019, #definimos el año
                            ...
                            #,destfile = "./eph_2019_T1.rds" #opcional: definimos dónde se va a guardar la base. 
                           )  
...
Aplicamos la función *pivot_wider()*:
...
De este modo, obtenemos una tabla de doble entrada que puede ser más fácil de leer en un informe o presentación.
...
Usualmente, vamos a recurrir a esta función cuando necesitemos seguir procesando la información (ya sea para otros cálculos o para graficar nuestros resultados).

# Unión de bases

Para unir dos bases de datos x e y a partir de un identificador común vamos a recurrir a distintas funciones join(x,y):

+ inner_join(x,y): el resultado incluye todas las filas que estén presentes tanto en x como en y.
+ left_join(x,y): el resultado incluye todas las filas que estén presentes en x (agregando las columnas de y con información sobre estas filas).
+ right_join(x,y): el resultado incluye todas las filas que estén presentes en y (agregando las columnas de x con información sobre estas filas).
+ full_join(x,y): el resultado incluye todas las filas que estén en x o y.
+ anti_join(x,y): el resultado incluye todas las filas de x que no estén presentes en y (sólo muestra las columnas de x).
...
Además de introducir esta nueva función, tomamos esta oportunidad para repasar algunas de las funciones con las que trabajamos en los encuentros anteriores.

```{r}
#En primer lugar, levantamos la base con la que vamos a trabajar y recodificamos las variables en base al código de registro:
...
tabla_2 <- base %>% 
  ...
  #creamos una nueva variable ESTADO_corregido, donde replicamos las categorías de ESTADO excepto en el caso de las amas de casa: en vez de clasificarlas como inactivas, las clasificamos como ocupadas.
  
  mutate(ESTADO_corregido = case_when(ESTADO == "Inactivo" & CAT_INAC == "Ama de casa" ~ "Ocupado",
                                      TRUE ~ paste0(ESTADO))) %>% 
  
  #Luego, calculamos la tasa de actividad para cada sexo en base a esta nueva variable.
  group_by(SEXO) %>% 
...
Ahora, vamos a recurrir a la nueva función *inner_join()* para unir las tablas y calcular la diferencia entre ambos cómputos:
...
#Ahora logramos convertirla al formato Date

```
El orden de los caracteres dmy_hms tiene que responder a la estructura de la información que intentamos convertir al formato fecha: si pedimos una estructura que no puede extraerse del input, no se convierte el string en una fecha.

```{r}

#cambiamos dmy por mdy

fecha_1_formato_mal <- mdy(fecha_1)
...
#En qué caso funcionaría la estructura mdy? Con un input en ese orden

fecha_1_v2 <- "abril 30 de 2021"

fecha_1_v2_formato <- mdy(fecha_1_v2)
...
llamados_atendidos_violencia_familiar_202202 <- read_excel("./llamados-atendidos-violencia-familiar-202202.xlsx") 
...
Los objetos que creamos en el entorno de R existen en el entorno de R y al cerrar la sesión, desaparecen. Sin embargo, podemos querer guardar algún resultado. A estos fines, podemos recurrir a las herramientas para exportar a excel nuestros resultados.

El comando más simple para hacer esto es write.xlsx() del paquete *openxlsx*:

```{r}
write.xlsx(tabla_1, file = "./prueba1.xlsx")
```
...
Existen otros paquetes para importar y exportar datos además de *openxlsx*, tales como *haven* y *foreign* (para archivos 'SPSS', 'Stata', 'SAS', etc.)
