...
También es una función muy útil en su variante `mutate_at()`para editar varias variables al mismo tiempo. Por ejemplo, supongamos que quiero pasar las variables de marca, dirección y localidad a minúscula (lo cual puedo hacer con la función `str_to_lower()`). 

```{r}
base_filtrada_1  <- base_filtrada_1 %>% mutate_at(vars(Marca, Dirección, Localidad), ~str_to_lower(.))
