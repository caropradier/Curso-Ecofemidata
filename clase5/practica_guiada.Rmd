...
Vamos a ello. En primer lugar, vamos a hacer lo que ya sabemos. Importamos la librería `tidyverse`, luego las liberías `stats` y `spatstat` para funciones adicionales y la librería `eph` que va a contener el paquete y la base de datos del primer trimestre de 2019 de la EPH.
...
base %>% group_by(REGION)%>%
        summarise(edad_media = weighted.mean(CH06, PONDIIO, na.rm=TRUE)) %>%
...
base %>% 
  ...
  coord_flip()+
        ...
        caption = "Encuesta Permanente de Hogares",
